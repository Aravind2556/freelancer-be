...
//     try {
//         const { email, name, phone, password, cpassword, role, qualification, skills, experience, linkedin, company } = req.body;

//         if (!email || !name || !phone || !password || !cpassword || !role || (role !== "Freelancer" ? (qualification || skills || experience || linkedin) : "") || (role !== 'Company' ? company : "")) {
//             return res.status(400).json({ success: false, message: "All fields are required." });
//         }

//         if (password !== cpassword) {
//             return res.status(400).json({ success: false, message: "Invalid password. Both passwords should match." });
//         }

//         // Check if user already exists
//         const isUser = await Register.findOne({ Email: email });
//         if (isUser) {
//             return res.status(400).json({ success: false, message: "This user is already registered. Please try again." });
//         }
...
//         return res.status(201).json({ success: true, message: "Registration successful", Role: req.session.profile });
            ...
            if (isfindLogin.Password === password) {
                if (isfindLogin.Role === userrole || isfindLogin.Role === "Admin") {
                    ...
                    req.session.save(err => {
                        ...
                        return res.json({ success: true, Role: isfindLogin.Role, message : "Login successfully" })
                    })
                }
                else {
                    return res.json({ success: false, message: "Access denied. Please log in with the correct user role." })
